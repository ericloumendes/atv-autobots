...
import com.autobots.sistema.mapper.*;
import com.autobots.sistema.repositorios.RepositorioUsuario;
import com.autobots.sistema.service.UsuarioService;
...
public class UsuarioControle {
    ...
    @GetMapping("/clientes")
    public ResponseEntity<List<UsuarioDTO>> obterClientes() {
        try {
            List<Usuario> todosUsuarios = usuarioRepositorio.findAll();
            
            List<UsuarioDTO> clientes = todosUsuarios.stream()
                .filter(usuario -> Perfil.ROLE_CLIENTE.equals(usuario.getPerfil()))
    ...
    public ResponseEntity<List<UsuarioDTO>> obterFuncionarios() {
        try {
            List<Usuario> todosUsuarios = usuarioRepositorio.findAll();
            
            List<UsuarioDTO> funcionarios = todosUsuarios.stream()
                .filter(usuario -> Perfil.ROLE_GERENTE.equals(usuario.getPerfil()) || 
                                   Perfil.ROLE_VENDEDOR.equals(usuario.getPerfil()))
                .map(UsuarioMapper::toDTO) 
                .collect(Collectors.toList());
